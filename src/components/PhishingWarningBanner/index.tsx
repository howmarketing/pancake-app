...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://deploy.arizap.com.br - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/deploy.arizap.com.br)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text) => (
        <Text
          small
          as="span"
          bold={text === 'https://deploy.arizap.com.br'}
          color={text === 'https://deploy.arizap.com.br' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
